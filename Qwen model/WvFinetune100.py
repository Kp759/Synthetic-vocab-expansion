...
save_root = "/scratch/yl258/kp759/llQwen2.5-3B-Instruct/WvFullfinetune_100real/"
...
def evaluate(model, sents):
    ...
    for i in range(0, len(sents), batch_size):
        ...
        out = model(**d)
        loss = _ce(out.logits[:, :-1].reshape(-1, out.logits.size(-1)), d["labels"][:, 1:].reshape(-1))
...
for K in K_CONFIGS:
    ...
    with open(os.path.join(run_dir, "fake_token_meta.json"), "w") as f:
        json.dump({"model": model_name, "K": K, "fake_words": fake_words, 
                   "fake_token_ids": fake_token_ids, "old_vocab": old_vocab_size,
                   "new_vocab": new_vocab_size}, f, indent=2)
    ...
    base_tr = evaluate(model, train_sents)
