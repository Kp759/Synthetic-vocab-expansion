...
save_root = "/scratch/yl258/kp759/llQwen2.5-3B/results_eng_es_prompt_tuning_fast"
...
for K in K_CONFIGS:
    ...
    # Baseline eval once
    base_es_test = evaluate(model, tokenizer, spanish_test_sents, prompt_length)
    ...
    with open(os.path.join(run_dir, "train.log"), "w", encoding="utf-8") as log_f:
        ...
        for epoch in range(epochs):
            ...
            if do_es:
                latest_es_test = evaluate(model, tokenizer, spanish_test_sents, prompt_length)
    ...
    # Final eval once
    final_es_test = evaluate(model, tokenizer, spanish_test_sents, prompt_length)
