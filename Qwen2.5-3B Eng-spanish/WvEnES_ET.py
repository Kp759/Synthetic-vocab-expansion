...
save_root = "/scratch/yl258/kp759/llQwen2.5-3B/results_eng_es_et_lm_fast"
os.makedirs(data_root, exist_ok=True)
os.makedirs(save_root, exist_ok=True)

spanish_dir = os.path.join(data_root, "spanish")
english_dir = os.path.join(data_root, "english")
os.makedirs(spanish_dir, exist_ok=True)
os.makedirs(english_dir, exist_ok=True)

spanish_train_path = os.path.join(spanish_dir, "train.txt")
spanish_test_path = os.path.join(spanish_dir, "test.txt")
english_test_path = os.path.join(english_dir, "test.txt")

AUTO_PREPARE_DATA = True
NUM_ES_TRAIN_SAVE = 6000
NUM_ES_TEST_SAVE = 1000
...
# Faster eval cadence
EVAL_ES_EVERY = 2   # Spanish test every 2 epochs
...
def build_local_wiki_files():
    print("Preparing local Spanish/English txt files from Hugging Face wikipedia...")
    ...
    for ex in ds_es:
        ...
        if len(txt) >= MIN_CHAR_LEN:
            es_texts.append(txt)
        if len(es_texts) >= (NUM_ES_TRAIN_SAVE + NUM_ES_TEST_SAVE):
            break
    ...
    if len(es_texts) < (NUM_ES_TRAIN_SAVE + NUM_ES_TEST_SAVE):
        raise RuntimeError(f"Need at least {NUM_ES_TRAIN_SAVE + NUM_ES_TEST_SAVE} Spanish examples, got {len(es_texts)}")
    if len(en_texts) < NUM_EN_TEST_SAVE:
        raise RuntimeError(f"Need at least {NUM_EN_TEST_SAVE} English examples, got {len(en_texts)}")

    save_lines(spanish_train_path, es_texts[:NUM_ES_TRAIN_SAVE])
    save_lines(spanish_test_path, es_texts[NUM_ES_TRAIN_SAVE:NUM_ES_TRAIN_SAVE + NUM_ES_TEST_SAVE])
    save_lines(english_test_path, en_texts[:NUM_EN_TEST_SAVE])

if AUTO_PREPARE_DATA:
    if not (os.path.exists(spanish_train_path) and os.path.exists(spanish_test_path) and os.path.exists(english_test_path)):
...
spanish_train_sents_all = load_lines(spanish_train_path)
spanish_test_sents = load_lines(spanish_test_path)
english_test_sents = load_lines(english_test_path)

print(f"Loaded {len(spanish_train_sents_all)} Spanish train examples")
print(f"Loaded {len(spanish_test_sents)} Spanish test examples")
print(f"Loaded {len(english_test_sents)} English test examples")

_word_pattern = re.compile(r"[A-Za-zÀ-ÿ]+(?:[-'][A-Za-zÀ-ÿ]+)?")
...
print(f"Selected {len(new_tokens)} new Spanish tokens")
print("Sample mined tokens:", new_tokens[:30])

if len(new_tokens) == 0:
    raise RuntimeError("No new Spanish tokens mined.")
...
for K in K_CONFIGS:
    spanish_train_sents = spanish_train_sents_all[:K]
    ...
    # Baseline eval once
    base_es_test = evaluate(model, tokenizer, spanish_test_sents)
    base_en_test = evaluate(model, tokenizer, english_test_sents)

    latest_es_test = base_es_test
    latest_en_test = base_en_test

    with open(log_path, "w", encoding="utf-8") as log_f:
        log_f.write("epoch,es_test_loss,en_test_loss\n")
        log_f.write(f"BASE,{base_es_test:.6f},{base_en_test:.6f}\n")
        log_f.flush()

        for epoch in range(epochs):
            random.shuffle(spanish_train_sents)
            model.train()

            for i in range(0, len(spanish_train_sents), batch_size):
                batch_texts = spanish_train_sents[i:i + batch_size]
            ...
            do_es = ((epoch + 1) % EVAL_ES_EVERY == 0) or (epoch == epochs - 1)
            ...
            if do_es:
                latest_es_test = evaluate(model, tokenizer, spanish_test_sents)
                es_msg = f"{latest_es_test:.4f}"

            if do_en:
                latest_en_test = evaluate(model, tokenizer, english_test_sents)
                en_msg = f"{latest_en_test:.4f}"

            print(f"[EPOCH {epoch:03d}] es_test={es_msg} | en_test={en_msg}")
            log_f.write(f"{epoch},{latest_es_test:.6f},{latest_en_test:.6f}\n")
            log_f.flush()

    # Final eval once
    final_es_test = evaluate(model, tokenizer, spanish_test_sents)
    ...
    result = {
        ...
        "eval_es_every": EVAL_ES_EVERY,
        "eval_en_every": EVAL_EN_EVERY,
        "base_es_test_loss": base_es_test,
        "base_en_test_loss": base_en_test,
        "final_es_test_loss": final_es_test,
