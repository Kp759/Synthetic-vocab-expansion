...
save_root = "/scratch/yl258/kp759/llPhi3.5-mini-Instruct/lora_k1000_Wv1/"
...
def evaluate(model, sents):
    ...
    for i in range(0, len(sents), batch_size):
        ...
        out = model(**d)
        loss = _ce(out.logits[:, :-1].reshape(-1, out.logits.size(-1)),
...
for K in K_CONFIGS:
    ...
    print(f"[PEFT] trainable params (should be LoRA): {trainable_params:,} | all params: {all_params:,} | trainable%: {100*trainable_params/all_params:.6f}%")

    lora_only = sum(p.numel() for n, p in model.named_parameters() if ("lora" in n.lower()) and p.requires_grad)
    print(f"[PEFT] LoRA-only trainable params (name filter): {lora_only:,}")
    ...
    log_f.write(f"BASE,| Train={base_train:.4f} | Test={base_test:.4f}\n")
