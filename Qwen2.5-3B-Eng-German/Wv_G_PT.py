...
save_root = "/scratch/yl258/kp759/llQwen2.5-3B/results_eng_de_prompt_tuning_fast"
...
for K in K_CONFIGS:
    ...
    # Baseline eval once
    base_de_test = evaluate(model, tokenizer, german_test_sents, prompt_length)
    ...
    with open(os.path.join(run_dir, "train.log"), "w", encoding="utf-8") as log_f:
        ...
        for epoch in range(epochs):
            ...
            if do_de:
                latest_de_test = evaluate(model, tokenizer, german_test_sents, prompt_length)
    ...
    # Final eval once
    final_de_test = evaluate(model, tokenizer, german_test_sents, prompt_length)
    final_en_test = evaluate(model, tokenizer, english_test_sents, prompt_length)

    result = {
        "method": "PromptTuning_FAST_DE",
        "seed": SEED,
        "model": model_name,
        "K": K,
        "epochs": epochs,
